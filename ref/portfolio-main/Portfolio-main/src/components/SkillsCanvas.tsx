...
const SKILLS: Skill[] = [
  ...
  { name: 'Prometheus',     cat: 'DevOps',   color: '#E6522C', logo: dv('prometheus/prometheus-original.svg') },
...
export default function SkillsCanvas() {
  ...
  useEffect(() => {
    ...
    // ── Helpers — use container size, not window ───────────────────────
    ...
    const ghostLineMats: THREE.LineBasicMaterial[] = [];
    ...
    for (let a = 0; a < cc.length; a++) {
      for (let b = a + 1; b < cc.length; b++) {
        ...
        const gMat = new THREE.LineBasicMaterial({ color: 0xffffff, transparent: true, opacity: .016 });
        grp.add(new THREE.Line(geo, gMat));
        ghostGeos.push(geo);
        ghostLineMats.push(gMat);
      }
    }

    // ── Theme-aware WebGL recoloring ──────────────────────────────────
    const isDark = () => document.documentElement.classList.contains('dark');

    const rebuildSprites = () => {
      const dark = isDark();
      const fgMain  = dark ? 'rgba(250,250,250,0.95)' : 'rgba(10,10,10,0.92)';
      Object.entries(CAT_META).forEach(([cat, m]) => {
        const sp = catSprites[cat]; if (!sp) return;
        const tex = (sp.material as THREE.SpriteMaterial).map as THREE.CanvasTexture;
    ...
    const applyThemeColors = () => {
      const dark = isDark();
      // Stars
      starMat.color.set(dark ? 0xffffff : 0x1a1a1a);
      starMat.opacity = dark ? 0.14 : 0.06;
      // Ghost inter-cluster links
      ghostLineMats.forEach(m => {
        m.color.set(dark ? 0xffffff : 0x111111);
        m.opacity = dark ? 0.016 : 0.030;
      });
      // Sprite labels
      rebuildSprites();
    };

    // Run once immediately + re-run on every theme class toggle
    applyThemeColors();
    ...
    function selectCat(cat: string) {
      ...
      // Offset targetX slightly negative so the cluster appears higher on screen, 
      // avoiding the bottom category pills.
      const targetX = -Math.atan2(c.y, Math.sqrt(c.x * c.x + c.z * c.z)) - 0.18;
    ...
    function showTT(i: number, mx: number, my: number) {
      ...
      if (ttRef.current) {
        ...
        const flip = rx > W() - 210;
        ttRef.current.style.left      = (flip ? rx - 12 : rx + 12) + 'px';
        ttRef.current.style.top       = ry + 'px';
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      // Fix: use canvas-relative coords for NDC, not raw clientX/Y
      const rect = wrap.getBoundingClientRect();
      ...
      if (hits.length) {
        ...
        if (idx === undefined) { hideTT(); return; }
        const s   = SKILLS[idx];
        const vis = !activeFilter || s.cat === activeFilter;
        if (vis) {
          hovIdx = idx;
    ...
    // ── Zoom mode: scroll only zooms when zoom-lock is active ─────────
    let zoomMode = false;

    const setZoomMode = (on: boolean) => {
      zoomMode = on;
      if (zoomBtnRef.current) {
        zoomBtnRef.current.classList.toggle('zoom-active', on);
        zoomBtnRef.current.title = on ? 'Disable 3D control (enable page scroll)' : 'Enable 3D control (orbit & zoom)';
        const textSpan = zoomBtnRef.current.querySelector('#zoom-text');
        if (textSpan) textSpan.textContent = on ? '3D CONTROL' : 'PAGE SCROLL';
        const iconSpan = zoomBtnRef.current.querySelector('#zoom-icon');
        if (iconSpan) iconSpan.textContent = on ? '⊕' : '⇕';
      }
      // Signal the CustomCursor
      window.dispatchEvent(new CustomEvent('skillsZoomMode', { detail: { active: on } }));
    };
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      e.preventDefault();
      e.stopPropagation();
      // Natural direction: scroll up (deltaY < 0) = zoom IN (decrease tZoom = camera closer)
      tZoom = Math.max(4, Math.min(18, tZoom + e.deltaY * .010));
    };

    const onZoomBtnClick = () => setZoomMode(!zoomMode);
    zoomBtnRef.current?.addEventListener('click', onZoomBtnClick);
    ...
    const onTouchMove = (e: TouchEvent) => {
      if (!zoomMode) return; // Allow natural page scrolling when not in interactive mode
    ...
    return () => {
      ...
      zoomBtnRef.current?.removeEventListener('click', onZoomBtnClick);
      resizeObserver.disconnect();
      themeObserver.disconnect();
      // Clear zoom mode signal on unmount
      window.dispatchEvent(new CustomEvent('skillsZoomMode', { detail: { active: false } }));
      ...
      ghostLineMats.forEach(m => m.dispose());
  ...
  return (
    ...
    >
      {/* Section title — matches site-wide pattern */}
      <div style={{ paddingTop: '5rem', paddingBottom: '1rem' }}>
        <ScrambleHeading label="Skills & Stack" plain="Tech" accent="Stack" />
      </div>
      ...
      >
        ...
        <div id="hint" ref={hintRef}>Drag to orbit &nbsp;·&nbsp; Click a category</div>
        <button id="reset-btn"   ref={resetBtnRef}>↩ &nbsp;Show all</button>
        <button id="zoom-btn"    ref={zoomBtnRef}  title="Enable zoom mode">
          <span id="zoom-icon">⇕</span>
          <span id="zoom-text">PAGE SCROLL</span>
        </button>
      </div>
    </div>
  );
